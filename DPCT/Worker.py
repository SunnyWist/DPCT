...
class Worker:
    ...
    def calculateGradient(self, rollout, bootstrap_value, episode_count, rnn_state0):
        ...
        discounted_rewards = discount(self.rewards_plus, self.conf.GAMMA)[:-1]
        self.value_plus = np.asarray(values.tolist() + [bootstrap_value])
        advantages = rewards + self.conf.GAMMA * self.value_plus[1:] - self.value_plus[:-1]
        advantages = discount(advantages, self.conf.GAMMA)

        num_samples = min(self.conf.EPISODE_SAMPLES, len(advantages))
        ...
        v_l, p_l, valid_l, e_l, g_n, v_n, grads = self.sess.run(
            [
                self.local_AC.value_loss,
    ...
    def imitation_learning_only(self, episode_count):
        ...
        for i in range(self.num_workers):
            ...
            # train_buffer: (state, vector, 最適行動, 報酬(常に1))
    ...
    def run_episode_multithreaded(self, episode_count, coord):
        if self.metaAgentID < self.conf.NUM_IL_META_AGENTS:
            ...
            self.imitation_learning_only(episode_count)
        ...
        with self.sess.as_default(), self.sess.graph.as_default():
            while self.shouldRun(coord, episode_count):
                ...
                if self.agentID == 1:
                    self.env._reset()
                    self.conf.joint_observations[self.metaAgentID] = self.env._observe()

                self.synchronize()  # synchronize starting time of the threads

                # Get Information For Each Agent
                validActions = self.env.listValidActions(
                    self.agentID, self.conf.joint_observations[self.metaAgentID][self.agentID]
                )

                s = self.conf.joint_observations[self.metaAgentID][self.agentID]

                rnn_state = self.local_AC.state_init
                rnn_state0 = rnn_state

                self.synchronize()  # synchronize starting time of the threads
                self.conf.swarm_reward[self.metaAgentID] = 0
                self.conf.swarm_targets[self.metaAgentID] = 0

                self.conf.episode_rewards[self.metaAgentID] = []
                self.conf.episode_finishes[self.metaAgentID] = []
                self.conf.episode_lengths[self.metaAgentID] = []
                self.conf.episode_mean_values[self.metaAgentID] = []
                self.conf.episode_invalid_ops[self.metaAgentID] = []
                self.conf.episode_stop_ops[self.metaAgentID] = []
                ...
                if True:
                    ...
                    if (
                        self.conf.OUTPUT_GIFS
                        and self.agentID == 1
                        and ((not self.conf.TRAINING) or (episode_count >= self.nextGIF))
                    ):
                        saveGIF = True
                        self.nextGIF = episode_count + self.conf.GIFS_FREQUENCY_RL
                    ...
                    while not self.env.finished:
                        a_dist, v, rnn_state = self.sess.run(
                            ...
                            feed_dict={
                                self.local_AC.inputs: [s[0]],  # state
                        ...
                        if not skipping_state:
                            if not (np.argmax(a_dist.flatten()) in validActions):
                                episode_inv_count += 1
                                train_val = 0
                            train_valid = np.zeros(self.conf.A_SIZE)
                            ...
                            self.conf.joint_actions[self.metaAgentID][self.agentID] = a
                            if a == 0:
                        ...
                        if self.agentID == 1:
                            all_obs, all_rewards = self.env.step_all(self.conf.joint_actions[self.metaAgentID])
                            for i in range(1, self.num_workers + 1):
                                self.conf.joint_observations[self.metaAgentID][i] = all_obs[i]
                                self.conf.joint_rewards[self.metaAgentID][i] = all_rewards[i]
                                self.conf.joint_done[self.metaAgentID][i] = self.env.world.agents[i].status == 1
                        ...
                        s1 = self.conf.joint_observations[self.metaAgentID][self.agentID]
                        r = copy.deepcopy(self.conf.joint_rewards[self.metaAgentID][self.agentID])
                        ...
                        if not skipping_state:
                            episode_buffer.append(
                                [
                                    s[0],
                                    a,
                                    self.conf.joint_rewards[self.metaAgentID][self.agentID],
                                    s1,
                                    v[0, 0],
                                    train_valid,
                                    s[1],
                        ...
                        if (len(episode_buffer) > 1) and (
                            (len(episode_buffer) % self.conf.EXPERIENCE_BUFFER_SIZE == 0)
                            or self.conf.joint_done[self.metaAgentID][self.agentID]
                            or episode_step_count == self.conf.MAX_EPISODE_LENGTH
                        ):
                            # Since we don't know what the true final return is,
                            # we "bootstrap" from our current value estimation.
                            if len(episode_buffer) >= self.conf.EXPERIENCE_BUFFER_SIZE:
                                train_buffer = episode_buffer[-self.conf.EXPERIENCE_BUFFER_SIZE :]
                            else:
                                train_buffer = episode_buffer[:]

                            if self.conf.joint_done[self.metaAgentID][self.agentID]:
                                s1Value = 0  # Terminal state
                                episode_buffer = []
                                self.conf.joint_done[self.metaAgentID][self.agentID] = False
                                targets_done += 1

                            else:
                                s1Value = self.sess.run(
                                    self.local_AC.value,
                                    feed_dict={
                            ...
                            self.loss_metrics, grads = self.calculateGradient(
                                train_buffer, s1Value, episode_count, rnn_state0
                            )
                        ...
                        if episode_step_count >= self.conf.MAX_EPISODE_LENGTH:
                            break

                    self.conf.episode_lengths[self.metaAgentID].append(episode_step_count)
                    self.conf.episode_mean_values[self.metaAgentID].append(np.nanmean(episode_values))
                    self.conf.episode_invalid_ops[self.metaAgentID].append(episode_inv_count)
                    self.conf.episode_stop_ops[self.metaAgentID].append(episode_stop_count)
                    self.conf.swarm_reward[self.metaAgentID] += episode_reward
                    self.conf.swarm_targets[self.metaAgentID] += targets_done

                    self.synchronize()
                    if self.agentID == 1:
                        self.conf.episode_rewards[self.metaAgentID].append(self.conf.swarm_reward[self.metaAgentID])
                        self.conf.episode_finishes[self.metaAgentID].append(self.conf.swarm_targets[self.metaAgentID])
                        ...
                        if saveGIF:
                            make_gif(
                                ...
                                "{}/episode_{:d}_{:d}_{:.1f}.gif".format(
                                    ...
                                    GIF_episode,
                                    episode_step_count,
                                    self.conf.swarm_reward[self.metaAgentID],
                                ),
                            )
    ...
    def work(self, currEpisode, coord, saver, allVariables):
        ...
        if self.conf.COMPUTE_TYPE == ParamatersConfig.COMPUTE_OPTIONS.multiThreaded:
    ...
    def parse_path(self, episode_count):
        ...
        # observation[0]はstateを表し、observation[1]はvectorを表す
        ...
        if np.random.rand() < self.conf.IL_GIF_PROB:
            saveGIF = True
        if saveGIF and self.conf.OUTPUT_IL_GIFS:
            GIF_frames = [self.env._render()]
        ...
        step_count = 0
        while step_count <= self.conf.IL_MAX_EP_LENGTH:
            ...
            none_on_goal = True
            path_step = 1
            while none_on_goal and step_count <= self.conf.IL_MAX_EP_LENGTH:
                completed_agents = []
                start_positions = []
                goals = []
                for i in range(self.num_workers):
                ...
                for i in range(self.num_workers):
                    ...
                    if self.env.world.agents[agent_id].status == 1:
                        ...
                        targets_done += 1
                        single_done = True
                        if targets_done % self.conf.MSTAR_CALL_FREQUENCY == 0:
                            new_MSTAR_call = True
                        else:
                            new_call = True
                if saveGIF and self.conf.OUTPUT_IL_GIFS:
        ...
        if saveGIF and self.conf.OUTPUT_IL_GIFS:
            make_gif(
                np.array(GIF_frames),
                "{}/episodeIL_{}.gif".format(self.conf.GIFS_PATH, episode_count),
            )
    ...
    def cpp_mstar_test(self):
        self.env._reset()
        rollouts, targets_done = self.parse_path(self.conf.EPISODE_COUNT)
        return rollouts, targets_done
